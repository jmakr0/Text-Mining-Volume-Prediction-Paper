...
Compared to other approaches, our $F_1$-score is $37\%$ higher than the  value by Tsagikas et al. and $8\%$ smaller than the values presented by Ambroselli et al. \cite{ambroselli2018prediction}.

\input{compare_results.tex}

Finally, we point out that our results were achieved by using another dataset. Ambroselli et al. is using a dataset that contains only articles by a German magazine. 
...
Further classification improvements could be achieved by taking additional features into account: 
- wie lange besucher auf der seite waren
- interaktion auf dem artikel (scrollen, klicks)
- placement auf landing page
- advertisement

Furthermore, a recent research approach by Le Cun shows that a very deep CNN architecture using character embeddings can be used for sentence classification REF.   
